...
class DGI(PreTrain):
    def __init__(self, *args, **kwargs):    # hid_dim=16
        ...
        self.optimizer = Adam(self.gnn.parameters(), lr=0.001, weight_decay = 10e-5)
    ...
    def load_data(self):
        if self.dataset_name in ['PubMed', 'CiteSeer', 'Cora','Computers', 'Photo', 'Reddit', 'WikiCS', 'Flickr', 'ogbn-arxiv','Actor', 'Texas', 'Wisconsin', 'nba', 'pokec_z', 'pokec_n', 'credit', 'bail', 'german']:
            data, input_dim, _, _ = load4node(self.dataset_name)
            self.input_dim = input_dim
            ...
            from torch_geometric import loader
            self.batch_dataloader = loader.DataLoader(graph_list,batch_size=128,shuffle=False)
    ...
    def pretrain_one_epoch(self):
        ...
        if self.dataset_name in ['PubMed', 'CiteSeer', 'Cora','Computers', 'Photo', 'Reddit', 'WikiCS', 'Flickr', 'ogbn-arxiv', 'nba', 'pokec_z', 'pokec_n', 'credit', 'Actor', 'german']:
            graph_original = self.graph_data
            graph_corrupted = copy.deepcopy(graph_original)
            idx_perm = np.random.permutation(graph_original.x.size(0))
            graph_corrupted.x = graph_original.x[idx_perm].to(self.device)
