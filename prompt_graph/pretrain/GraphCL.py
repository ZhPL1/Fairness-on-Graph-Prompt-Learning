...
class GraphCL(PreTrain):
    def __init__(self, graph_list, input_dim, out_dim, *args, **kwargs):    # hid_dim=16
    ...
    def load_graph_data(self):
        ...
        if self.dataset_name in ['PubMed', 'CiteSeer', 'Cora','Computers', 'Photo', 'Reddit', 'WikiCS', 'Flickr', 'ogbn-arxiv','Actor', 'Texas', 'Wisconsin', 'nba', 'pokec_z', 'pokec_n', 'creditA', 'credit', 'bail', 'german']:
            # self.graph_list = NodePretrain(data = self.graph_list, num_parts=100, split_method='Cluster')
            self.graph_list = NodePretrain(data = self.graph_list, num_parts=0, split_method='Random Walk')
        else:
