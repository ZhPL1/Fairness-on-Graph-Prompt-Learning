...
def process_tu(data,class_num,node_class):
    ...
    for g in range(nb_graphs):
        if g == 0:
            ...
            # coo = sp.coo_matrix((np.ones(e_ind.shape[1]), (e_ind[0, :].numpy(), e_ind[1, :].numpy())), shape=(features.shape[0], features.shape[0]))
            ...
            #     features = np.vstack((features  ,tmpfeature.numpy()))
            # else:
            #     features = np.row_stack((features.numpy(),tmpfeature.numpy()))
            features = np.row_stack((features,tmpfeature))
            tmplabel = data[g].y[0]
            # rawlabels = np.row_stack((rawlabels.numpy(),tmplabel.numpy()))
            rawlabels = np.row_stack((rawlabels,tmplabel))
            e_ind = data[g].edge_index
            # coo = sp.coo_matrix((np.ones(e_ind.shape[1]), (e_ind[0, :].numpy(), e_ind[1, :].numpy())), shape=(tmpfeature.shape[0], tmpfeature.shape[0]))
